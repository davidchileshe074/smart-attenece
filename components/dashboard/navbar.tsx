  ...
  return (
    <header className="h-16 bg-white/80 backdrop-blur-md border-b border-slate-100 fixed top-0 right-0 left-0 md:left-64 z-40 px-6 flex items-center justify-between">
      ...
      <form
        ...
        className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-slate-100 rounded-md border border-slate-200 text-slate-500 w-64 hover:bg-slate-200 transition-colors"
      >
        <Search className="h-4 w-4" />
        <input
          value={searchQuery}
          onChange={(event) => setSearchQuery(event.target.value)}
          className="w-full bg-transparent text-xs font-medium text-text-primary placeholder:text-slate-500 outline-none"
          placeholder="Search courses or sessions..."
          aria-label="Search dashboard"
        />
        <button
          type="submit"
          className="ml-auto text-[10px] bg-white px-1.5 py-0.5 rounded border border-slate-300 text-slate-600"
        >
          Search
        </button>
      </form>
      ...
      <div className="flex items-center gap-4">
        <Link
          ...
          className="h-8 w-8 flex items-center justify-center rounded-md hover:bg-slate-100 transition-colors relative"
        ...
        <Link href="/dashboard/settings" className="flex items-center gap-3 cursor-pointer group" aria-label="Open settings">
